...
class SimpleIOSandboxedProc(SandboxedProc):
    ...
    def communicate(self, input=None):
        ...
        try:
            ...
            sys.stderr.write(self._error.getvalue())
            raise
...
class SimpleSandboxedProc(VirtualizedSandboxedProc, SimpleIOSandboxedProc):
    """
    Simple sandboxed proc that runs a (python) script in a sandbox

    The passed extras are also available to the sandboxed process (readonly)
    """
    ...
    def wait(self):
        self.returncode = super(SimpleSandboxedProc, self).wait()
    ...
    def build_virtual_root(self):
        ...
        return Dir({
            'bin': Dir({
                ...
                'lib-python': RealDir(os.path.join(self.libroot, 'lib-python'),
                    exclude=exclude),
...
try:
    ...
    func = %(module_name)s.%(func_name)s
    data = simplejson.load(sys.stdin)
    result = func(*data['args'], **data['kwargs'])
    simplejson.dump({'error': False, 'data': result}, sys.__stdout__)
except:
...
def simple_call(sandbox_executable, script_path, func_name, args=None, kwargs=None, extra=None):
    """
    Calls a function in a sandbox and returns the functions return value.

    @param sandbox_executable: Path to the sandbox pypy excecutable
    @param script_path: Path to the script in which the function is defined
    @param func_name: Function name, must be at the top level of the script provided
